...
def follow_graph():
    ...
    pub_stage = rospy.Publisher("/robot_1/cmd_vel", Twist, queue_size=1)
    rospy.init_node("follow_graph_1")
    rospy.Subscriber("/robot_1/base_pose_ground_truth", Odometry, callback_pose)
    rospy.Subscriber("/robot_1/base_scan", LaserScan, callback_laser)
    ...
    #----------
    Hole_path = [30, 1, 2, 14, 13, 12, 10, 11, 35, 9, 8, 36, 34, 15, 33, 16, 17, 18, 19, 20, 32, 21, 22, 31, 29, 28, 30, 24,
...
if __name__ == '__main__':
    ...
    rp = rospkg.RosPack()
    path = rp.get_path('centralized')
    path = path + '/text/path_robot1.txt'
    file_path = open(path, 'w')
    path = rp.get_path('centralized')
    path = path + '/text/vel_robot1.txt'
